            ...
            <div className="hidden md:block">
                <div className={`${styles.section}`}>
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-3 py-3 md:py-5">
                        ...
                        {/* Search box */}
                        <div className="order-4 sm:order-3 basis-full sm:basis-0 sm:flex-1 relative mx-15">
                            <input
                                ref={searchRef}
                                type="text"
                            ...
                            {search && searchData && searchData.length !== 0 ? (
                                <div className="absolute w-full min-h-[30vh] bg-slate-50 shadow-sm-2 z-20 p-4">
                                    {searchData.map((i, index) => {
                                        const product_name = i?.productName.replace(/\s+/g, "-");
                                        return (
                                            <Link to={`/product/${i._id}`} key={index}>
                                                <div className="w-full flex items-start py-3" onClick={() => setSearch("")}>
                                                    <img
                                                        src={i?.images[0]?.url}
                                                        alt=""
                ...
                >
                    ...
                    >
                        ...
                        <div className={`${styles.normalFlex} gap-3 md:gap-4 lg:gap-5`}>
                            <div className="relative cursor-pointer" onClick={() => setOpenWishlist(!openWishlist)}>
                                <AiOutlineHeart size={26} className="text-[#fff]" />
                            ...
                            <div className="relative cursor-pointer" onClick={() => setOpenCart(!openCart)}>
                                <AiOutlineShoppingCart size={26} className="text-[#fff]" />
                                <span className="absolute -right-1 -top-1 w-4 h-4 rounded-full bg-[#3bc177] text-white font-mono text-[12px] leading-4 text-center">
                            ...
                            {isAuthenticated ? (
                                ...
                                <Link to={"/log-in"}>
                                    <CgProfile size={26} className="text-[#fff]" />
                                </Link>
                            )}
                            {/* {Cart POP Up} */}
                            {
                                        ...
                                        {search && searchData && searchData.length !== 0 ? (
                                            <div className="absolute w-full left-0 min-h-[30vh] bg-slate-50 shadow-sm-2 z-20 p-3">
                                                {searchData.map((i, index) => {
                                                    ...
                                                    return (
                                                        <Link to={`/product/${i._id}`} key={index}>
                                                            <div className="w-full flex items-center py-3" onClick={() => setSearch("")}>
                                                                <img
                                                                    src={i?.images[0]?.url}
                                                                    alt=""
